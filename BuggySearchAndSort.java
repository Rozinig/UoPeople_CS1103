...
public class BuggySearchAndSort {
	...
	public static boolean contains(int[] array, int val) {
		for (int i = 0; i < array.length; i++) {
			...
			/*else
				return false;
				This else and return statement means that the subroutine will always finish on the first
				run of the for loop and not check the other values.
				*/
		}
		return false;
	}
	...
	public static void bubbleSort(int[] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array.length-1; j++) {// typed an i++ instead of a j++ so the inner for loop will never complete as j never increases
	...
	public static void selectionSort(int[] array) {
		for (int top = array.length - 1; top > 0; top--) {
			int positionOfMax = 0;
			for (int i = 1; i <= top; i++) {
				if (array[i] > array[positionOfMax])// was always checking against array[1] instead of array[i]
					positionOfMax = i;
			}
	...
	public static void insertionSort(int[] array) {
		for (int top = 1; top < array.length; top++) {
			...
			int pos = top - 1;
			while (pos >= 0 && array[pos] > temp) {// Needed the equals sign 
				   // move items that are bigger than temp up one position
				array[pos+1] = array[pos];
				pos--;
			}
			array[pos+1] = temp;  // place temp into last vacated position and a plus 1
		}
	}
