	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (theStrings.size() != 0)
				for (DrawTextItem text : theStrings){
				text.draw(g);
				}
		}
	}
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			try {
				...
				File file = fileChooser.getOutputFile(this, "Select File Name", "text.dti");
				if (file ==null)
						return;
				...
				for (DrawTextItem text : theStrings){
					out.println(text.getString());
					out.println(text.getX());
					out.println(text.getY());
					out.println(text.getTextColor().getRed());
					out.println(text.getTextColor().getGreen());
					out.println(text.getTextColor().getBlue());
					}
			...
			try {
				...
				while (true) {
					...
					int blue = Integer.parseInt(in.nextLine());
					Color c = new Color(red, green, blue);
					DrawTextItem text = new DrawTextItem(next);
					text.setX(x);
					text.setY(y);
					text.setTextColor(c);
					theStrings.add(text);		
			...
			theStrings.remove(theStrings.size()-1);   // The most recent string from the canvas.
			undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
