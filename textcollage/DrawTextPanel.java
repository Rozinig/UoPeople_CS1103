...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem randomItem = new JMenuItem("Random Text ...");
			randomItem.setAccelerator(KeyStroke.getKeyStroke(commandKey + "R"));
			randomItem.addActionListener(menuHandler);
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			...
			try {
				...
				for(DrawTextItem word : theString) {
					p.println(word.getString()); // the text
					p.println(word.getX()); // the coordinate of X of text
					p.println(word.getY()); // the coordinate of Y of text
					Color textcolor = word.getTextColor();
					p.println(textcolor.getRGB()); // save the color of the text
				}
				p.close();
			}
			catch(Exception e) {
				JOptionPane.showMessageDialog(this, "There is error while saving the file. /n The error is : "+ e );
			}
		}
			...
			try {
				...
				while(scanner.hasNext()) {
					...
					DrawTextItem word = new DrawTextItem(text);
					word.setX(Integer.parseInt(scanner.next()));
					scanner.nextLine();
					word.setY(Integer.parseInt(scanner.next()));
					scanner.nextLine();
					Color textColor = Color.decode(scanner.next());
			...
			JOptionPane.showMessageDialog(this, "There is error while opening the file. /n The error is : "+ e);
		}
		}
		else if (command.equals("Random Text ...")) {
			SimpleFileChooser fileChooser = new SimpleFileChooser();
			File inputFile = fileChooser.getInputFile();
			try {
				theString.clear();
				...
				while(scanner.hasNext()) {
					...
					String text = scanner.nextLine();
					DrawTextItem word = new DrawTextItem(text);
					word.setX(rand.nextInt(600));
					word.setY(rand.nextInt(700));
					word.setTextColor(new Color(r, g, b));
					word.setRotationAngle(rand.nextInt(360));
					word.setMagnification(rand.nextInt(4) + 1);
					theString.add(word);
			}
			...
			JOptionPane.showMessageDialog(this, "There is error : " + e);
		}
		}
